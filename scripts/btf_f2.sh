...
CUDA_VISIBLE_DEVICES=1 uv run python -m ai_challenge.method.distill --student Qwen/Qwen2.5-0.5B --bf16 \
  --temperature 3 --alpha 0.25 --max-length 640 --serialize btrailf --teacher-logits qgptoss qw6 q7bteam \
  --val-fold 2 --out runs/kd_btf_f2 > runs/kd_btf_f2.log 2>&1
echo "[btf_f2] OOF=$(grep -oE '"macro_f1": [0-9.]+' runs/kd_btf_f2/metrics.json 2>/dev/null|grep -oE '0\.[0-9]+') (기준 f2=0.79367)" >> runs/weights_lab.log
